%ytraj=traj_test;
ytraj=[0*traj_testf(1)+curr_mode;traj_testf];
%%
close all;
l=p1.l;
d=p1.d;
...
for i=1:1%length(ytraj.traj)
    yHy=ytraj;%.traj{i};
    t = linspace(yHy.tspan(1),yHy.tspan(end),50);%yHy.breaks;%linspace(yHy.tspan(1),yHy.tspan(end),50);%
    y = yHy.eval(t);
    %
    figure(1)
    plot(t,y(1,:)), hold on
