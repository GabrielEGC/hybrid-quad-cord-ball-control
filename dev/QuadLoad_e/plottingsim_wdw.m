...
ytraj=xtraj;
%
save(['UTPCdone' mat2str(isav)],'utraj')
save(['YTPCdone' mat2str(isav)],'ytraj')
pause(0.1)
...
for isav=6:100
p = QuadLoadPlant2D2I();
load(['UTPCdone' mat2str(isav)])
load(['YTPCdone' mat2str(isav)])
l=p.l;
d=p.d;

figure(3)
xs=1.5:0.01:2.5;
ys=0.25*sqrt(((xs-2)/0.1).^4+1);
ys2=-0.25*sqrt(((xs-2)/0.1).^4+1);
...
for i=1:length(ytraj.traj)
    yHy=ytraj.traj{i};
    t = yHy.breaks;%linspace(yHy.tspan(1),yHy.tspan(end),50);%
    y = yHy.eval(t);
    %
    figure(1)
    plot(t,y(1,:)), hold on
    ...
    figure(6)
    plot(t,y(1,:)), hold on
    plot(t,sqrt((y(2,:)-y(5,:)).^2+(y(3,:)-y(6,:)).^2))
    legend('m','Length')
    pause
end
%%
