function [p,utraj,xtraj,z,traj_opt,F,info]=runDircolBnB2I
% from an initial balancing condition, take one step and return to the
% balance upright.

p = BnBPlant2D2I();
N = [15;15;15]; %Knot points First nd Second Mode

options.u_const_across_transitions = true; %set
options.periodic = false; %Supposing another constraint x(tf)=x(0)

traj_opt = HybridTrajectoryOptimization(@DircolTrajectoryOptimization, p,[1;2;1],N,{[0.5 4],[0.5 4],[0.5 4]},options); %Test 10

x0 = [ 2; 0; 0; 0; pi/2;   0; 0; 0; 0; 3];

t1 = 0.6;
x1 = [2; 0; 2; 1; 0;      0; 0; 0; 0; 0];
