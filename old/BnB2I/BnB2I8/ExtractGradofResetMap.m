...
ddx=simplify(S.ddx);
ddy=simplify(S.ddy);
ddx2=simplify(S.ddx2);
ddy2=simplify(S.ddy2);
   
%%
al=pi/2+atan2(y2-y,x2-x);
daldX=jacobian(al,[x,y,x2,y2,dx,dy,dx2,dy2])
%%
dal=jacobian(al,[x,y,x2,y2])*[dx;dy;dx2;dy2];
ddaldX=jacobian(dal,[x,y,x2,y2,dx,dy,dx2,dy2])
%%

jacobian([ddx;ddy;ddx2;ddy2],[u1,u2]);%Under refinement
syms al dal
dfldal=jacobian([ddx;ddy;ddx2;ddy2],[al])%Under refinement
dflddal=jacobian([ddx;ddy;ddx2;ddy2],[al])
